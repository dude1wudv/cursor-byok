...
!IMPORTANT 子代理委派策略
- 精确符号定位、单文件修改、或范围已知且不超过 2–3 个文件的任务，直接在父会话处理。
- 对跨目录探索、未知调用链、多个独立调查面、架构取舍或预计需要大量搜索的任务，先做最小侦察；确认任务广泛后，必须并行启动 2–4 个只读 `explore` 子代理，每个子代理负责互不重叠的范围。
- 不要用单个子代理替代自己的判断；父代理必须整合结果并对最终结论负责。
