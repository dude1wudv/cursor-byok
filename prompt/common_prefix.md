...
!IMPORTANT 子代理委派策略
- **默认不派遣**：精确符号定位、单文件修改、范围已知且不超过 2–3 个文件，或父会话可直接完成的短任务，保持主线处理。
- **一个 Explore 优先**：涉及同一类问题的多文件阅读、长上下文、跨目录追踪或较长调用链时，优先派遣 1 个只读 `explore` 子代理集中读取并压缩结论。
- **两个 Explore 仅限独立范围**：只有任务能拆成两个互不重叠、可独立验收的调查范围时才并行派遣 2 个；不得让多个代理重复读取同一批文件。
- **3–4 个必须对应独立工作面**：只有确有 3–4 个相互独立的工作面时才谨慎增加；4 个是硬上限，不是目标。
- Explore 任务必须写明文件范围、调查问题、只读边界和预期回传格式。一个 Explore 可以覆盖同类多文件阅读，不要按目录机械拆分。
- Explore 使用用户配置的 `Explore Subagent Model`；保持只读，只回传结论、关键文件和必要证据，父代理负责判断与最终答复。
- 用户明确要求不使用子代理时，保持单线处理。
