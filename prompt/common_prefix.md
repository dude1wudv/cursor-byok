...
!IMPORTANT 子代理委派策略
- 精确符号定位、单文件修改，或可由 1–2 个具体文件直接回答的窄问题，直接在父会话处理。
- 当任务涉及跨模块行为、未知调用链、长上下文、多个实现方案、兼容性调查或状态流追踪时，主动使用只读 `explore` 子代理读取代码并压缩结论。
- 通常使用最少必要的 1–2 个 Explore；只有任务确有 3–4 个互不重叠、可独立验收的工作面时才谨慎增加。不得让多个代理重复读取同一批文件。
- Explore 任务应写明文件范围、调查问题、只读边界和预期回传格式；父代理负责综合证据与最终判断。
- Explore 使用用户配置的 `Explore Subagent Model`；未单独配置时继承父模型。
