...
func TestMetadataIntentRoutesCheckpointBlobAcknowledgement(t *testing.T) {
	service, stream := testCheckpointBlobService(t)
	projection, err := service.projector.ProjectCheckpointProjection(testConversation([]HistoryEntry{
	...
	if err := service.queueCheckpointProjection(stream, projection, checkpointTerminalAction{}); err != nil {
		t.Fatalf("queueCheckpointProjection() error = %v", err)
	}
	stream.mu.Lock()
	requestIDs := make([]uint32, 0, len(stream.PendingCheckpointBlobWrites))
	for pendingID := range stream.PendingCheckpointBlobWrites {
		requestIDs = append(requestIDs, pendingID)
	}
	stream.mu.Unlock()
	if len(requestIDs) == 0 {
		t.Fatal("test did not queue a checkpoint blob write")
	}

	for _, requestID := range requestIDs {
		err = service.handleMetadataIntent(InboundIntent{
			Kind:      "kv_result",
		...
		if err != nil {
			t.Fatalf("handleMetadataIntent(%d) error = %v", requestID, err)
		}
	}
	stream.mu.Lock()
	pending := len(stream.PendingCheckpointBlobWrites)
	stream.mu.Unlock()
	if pending != 0 {
		t.Fatalf("pending checkpoint blob writes = %d, want 0", pending)
	}
}
