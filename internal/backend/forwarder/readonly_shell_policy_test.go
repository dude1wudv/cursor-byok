...
func TestValidateReadonlyGitCommandWhitelist(t *testing.T) {
	tests := []struct {
		...
		{name: "show-ref allowed", command: "git show-ref --heads"},
		{name: "for-each-ref allowed", command: `git for-each-ref --format="%(refname)" refs/heads`},
		{name: "rev-list allowed", command: "git rev-list --max-count=3 HEAD -- path/file.go"},
		{name: "name-rev allowed", command: "git name-rev HEAD"},
		{name: "symbolic-ref read allowed", command: "git symbolic-ref HEAD"},
		{name: "symbolic-ref write rejected", command: "git symbolic-ref HEAD refs/heads/main", wantErr: "only allows reading"},
		{name: "cat-file read allowed", command: "git cat-file -p HEAD"},
		{name: "cat-file textconv rejected", command: "git cat-file --textconv HEAD:file", wantErr: "cat-file flag"},
		{name: "config override rejected", command: "git -c core.pager=cat status", wantErr: "configuration overrides"},
		{name: "config env rejected", command: "git --config-env=core.pager=PAGER status", wantErr: "configuration overrides"},
		{name: "diff no-index rejected", command: "git diff --no-index a b", wantErr: "flag"},
