...
// 同时从 schema 中删除 notify_on_output/profile：模型看不到的字段就不会填写，
// 服务端剥离逻辑只兜底旧缓存请求（曾因 schema 宣告 + 服务端拒绝的矛盾导致模型无限重试被 Skipped）。
